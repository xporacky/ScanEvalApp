...
if grep -iq 'fedora' /etc/os-release; then
  ...
  sudo dnf install -y mesa-libEGL-devel libxkbcommon-devel wayland-devel vulkan-headers vulkan-loader-devel libxkbcommon-x11-devel
  ...
  sudo apt install -y libegl1-mesa-dev libxkbcommon-dev libwayland-dev libvulkan-dev libxkbcommon-x11-dev
#
fi
