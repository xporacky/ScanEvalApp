...
if grep -iq 'fedora' /etc/os-release; then
  ...
  sudo dnf install libxkbcommon-devel wayland-devel vulkan-headers vulkan-loader-devel libxkbcommon-x11-devel
  ...
  sudo apt install -y libxkbcommon-dev libwayland-dev libvulkan-dev libxkbcommon-x11-dev
#
fi
