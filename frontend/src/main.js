...
async function refreshData() {
  ...
  try {
    data.exams = (await ListExams()) || [];
  } catch (err) {
    console.error('ListExams failed', err);
  }

  try {
    data.students = (await ListStudents()) || [];
  } catch (err) {
    console.error('ListStudents failed', err);
  }

  try {
    data.configs = (await ListConfigs()) || [];
  } catch (err) {
    console.error('ListConfigs failed', err);
  }

  try {
    data.savePath = (await GetSavePath()) || '';
  } catch (err) {
    console.error('GetSavePath failed', err);
  }
