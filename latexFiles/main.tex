...
\usepackage{graphicx}
% todo
\setlength{\headheight}{45.90973pt}  % Nastavenie výšky hlavičky
\addtolength{\topmargin}{-3.20332pt} % Kompenzácia horného okraja
\pagestyle{fancy}

\fancyhead[L]{ % Logo on the left
   % \hspace{1cm} % Add horizontal space to push the logo to the right
    % \parbox{\textwidth}{
    \raisebox{1ex}{\includegraphics[width=2.1cm]{latexFiles/stu-fei-vector-logo.png}}
    % }
}
...
\fancyhead[R]{
    \raisebox{5ex}{
        \scalebox{0.6}{% Zmenši QR kód na 50 % pôvodnej veľkosti
           \qrcode{ {{.QrCode}} }
        }
    }
}
...
\newcommand{\generateBlock}[2]{ % Takes `start` and `end` question numbers
    ...
    \begin{tikzpicture}[font=\small]
        \draw[thick, black] (-1, -22) rectangle (11.5, -1.5); % Outer rectangle


        % Header labels
        \foreach \position/\letter in {1/A, 2/B, 3/C, 4/D, 5/E} {
            \node at ({\position * 2}, -1) {\letter};
        }
