...
func UpdateStudentAnswers(db *gorm.DB, studentId uint, examId uint, questionNumber int, answers []rune, pageNumber int, correctAnswers []rune) error {
...
if len(studentAnswers) < exam.QuestionCount {
studentAnswers = make([]rune, exam.QuestionCount)
}
for i, answer := range answers {
idx := (questionNumber - len(answers)) + i + 1
if idx >= 0 && idx < len(studentAnswers) {
...
if i < 0 || i >= len(studentAnswers) || i >= len(correctAnswers) {
continue
}
...
return UpdateStudent(db, student)
}
