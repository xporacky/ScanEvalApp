...
func ProcessPage(doc *fitz.Document, pageNumber int, exam *models.Exam, db *gorm.DB, progressChan chan string, totalPages int, counter *int, failedPages *FailedPages) {
    ...
    questionNumber, answers, versionCode := EvaluateAnswers(&mat, exam.QuestionCount, choices, pageNumber)
    logger.Info(fmt.Sprintf("Page %d : %s", pageNumber+1, versionCode), "studentID", student.ID)
    fmt.Printf("Page %d : %s (Student ID: %d)\n", pageNumber+1, versionCode, student.ID)
    ...
    err = repository.UpdateStudentAnswers(db, student.ID, exam.ID, questionNumber, answers, pageNumber+1,[]rune(versionCode))
    mutexUpdate.Unlock()
