...
//   - pageNumber: The page number for debugging PNG saving.
//
// Returns:
...
//   - string: The version code detected from the header.
func EvaluateAnswers(mat *gocv.Mat, numberOfQuestions int, numberOfChoices int, pageNumber int) (int, []rune, string) {
    ...
    group := GetGroupCode(&croppedUpperHeader, pageNumber)
    ...
    subGroup := GetSubGroupCode(&croppedLowerHeader, pageNumber)
    ...
    for i := 0; i < NUMBER_OF_QUESTIONS_PER_PAGE; i++ {
        studentAnswers = append(studentAnswers, GetAnswer(&croppedMat, i, numberOfChoices, pageNumber))
    ...
    *mat = croppedMat
    ...
    return questionNumber - 1, studentAnswers, groupCode
}
...
func GetAnswer(mat *gocv.Mat, i int, numberOfChoices int, pageNumber int) rune {
    answer := rune('x')
    state := StateEmpty
    ...
    for j := 1; j <= numberOfChoices; j++ {
        ...
        if rect.Empty() {
            if pageNumber == 68 {
                saveSquarePNG(&checkboxMat, fmt.Sprintf("answer%d_%d", i+1, j), false)
            }
            checkboxMat.Close()
        ...
        // If no X found and state is empty, check for circle in border
        if state == StateEmpty && detectCircleInBorder(&checkboxMat, rect) {
            if pageNumber == 68 {
                saveSquarePNG(&checkboxMat, fmt.Sprintf("answer%d_%d_circle", i+1, j), false)
            }
            rectMat.Close()
            checkboxMat.Close()
            answer = rune('a' + (j - 1))
...
// detectCircleInBorder checks the border area around a square for circle marks using density similar to X detection
func detectCircleInBorder(checkboxMat *gocv.Mat, rect image.Rectangle) bool {
    margin := 5
    borderRects := []image.Rectangle{
        // Top border
        {Min: image.Point{rect.Min.X - margin, rect.Min.Y - margin}, Max: image.Point{rect.Max.X + margin, rect.Min.Y}},
        // Bottom border
        {Min: image.Point{rect.Min.X - margin, rect.Max.Y}, Max: image.Point{rect.Max.X + margin, rect.Max.Y + margin}},
        // Left border
        {Min: image.Point{rect.Min.X - margin, rect.Min.Y - margin}, Max: image.Point{rect.Min.X, rect.Max.Y + margin}},
        // Right border
        {Min: image.Point{rect.Max.X, rect.Min.Y - margin}, Max: image.Point{rect.Max.X + margin, rect.Max.Y + margin}},
    }

    markedBorders := 0
    for _, borderRect := range borderRects {
        if borderRect.Min.X < 0 {
            borderRect.Min.X = 0
        }
        if borderRect.Min.Y < 0 {
            borderRect.Min.Y = 0
        }
        if borderRect.Max.X > checkboxMat.Cols() {
            borderRect.Max.X = checkboxMat.Cols()
        }
        if borderRect.Max.Y > checkboxMat.Rows() {
            borderRect.Max.Y = checkboxMat.Rows()
        }

        if borderRect.Dx() <= 0 || borderRect.Dy() <= 0 {
            continue
        }

        borderMat := checkboxMat.Region(borderRect)
        meanIntensity := borderMat.Mean()
        borderMat.Close()

        // Check if border has dark pixels (circle mark)
        if meanIntensity.Val1 < MEAN_INTENSITY_X_HIGHEST && meanIntensity.Val1 > MEAN_INTENSITY_X_LOWEST {
            markedBorders++
        }
    }

    // Need at least 2 borders to be marked to consider it a circle
    return markedBorders >= 2
}

func saveSquarePNG(mat *gocv.Mat, filename string, _ bool) {
    filePath := fmt.Sprintf("./assets/tmp/%s.png", filename)
    SaveMat(filePath, *mat)
}

func GetGroupCode(headerMat *gocv.Mat, pageNumber int) rune {
    state := StateEmpty
    answer := rune('x')
    numberOfGroups := 8
    ...
    for j := 0; j < numberOfGroups; j++ {
        ...
        if rect.Empty() {
            if pageNumber == 68 {
                saveSquarePNG(&checkboxMat, fmt.Sprintf("group_%d", j+1), false)
            }
            checkboxMat.Close()
            if state == StateCircleFound {
                return rune('x')
        ...
        if state == StateEmpty && detectCircleInBorder(&checkboxMat, rect) {
            if pageNumber == 68 {
                saveSquarePNG(&checkboxMat, fmt.Sprintf("group_%d_circle", j+1), false)
            }
            rectMat.Close()
            checkboxMat.Close()
            answer = rune('a' + j)
...
func GetSubGroupCode(headerMat *gocv.Mat, pageNumber int) int {
    state := StateEmpty
    answer := -1
    numberOfSubgroups := 5
    ...
    for j := 0; j < numberOfSubgroups; j++ {
        ...
        if rect.Empty() {
            if pageNumber == 68 {
                saveSquarePNG(&checkboxMat, fmt.Sprintf("subgroup_%d", j+1), false)
            }
            checkboxMat.Close()
            if state == StateCircleFound {
                return -1
        ...
        if state == StateEmpty && detectCircleInBorder(&checkboxMat, rect) {
            if pageNumber == 68 {
                saveSquarePNG(&checkboxMat, fmt.Sprintf("subgroup_%d_circle", j+1), false)
            }
            rectMat.Close()
            checkboxMat.Close()
            answer = j + 1
