...
func (t *UploadCsv) CreateExam(gtx layout.Context, th *themeUI.Theme, db *gorm.DB, tm *tabmanager.TabManager) layout.Dimensions {
	...
	// Define options for selector (3-8)
	optionValues := []string{"3", "4", "5", "6", "7", "8"}

	// Initialize selector if not done
	if optionSelector.Theme == nil {
				...
				return layout.Flex{Axis: layout.Horizontal}.Layout(gtx,
					layout.Flexed(columnWidths[0], func(gtx layout.Context) layout.Dimensions {
						return layout.UniformInset(insetwidth).Layout(gtx, func(gtx layout.Context) layout.Dimensions {
							editor := widgets.NewEditorField(th.Theme, &nameInput, "Názov")
...
func renderOptions(gtx layout.Context, th *themeUI.Theme, questionIndex int, qf *questionForm) []layout.FlexChild {
	// Get the actual option count (3-8)
	optionCount := optionCountInput + 3 // Convert index to actual count

	// Generate options dynamically based on count
	options := make([]string, optionCount)
	for i := 0; i < optionCount; i++ {
		options[i] = string(rune('A' + i)) // A, B, C, D, E, F, G, H
	}
...
func submitForm(db *gorm.DB, t *UploadCsv, tm *tabmanager.TabManager) {
	...
	logger.Info("Test bol úspešne vytvorený",
		...
		slog.Int("optionCount", exam.OptionCount))
	...
	// Reset option count to default (5 options)
	optionCountInput = 2 // This corresponds to 5 options (index 2 in 0-5 range)

	// Reset questions
	questionForms = nil
	tm.ActiveTab = 0
	logger.Info("Pocet moznosti:", slog.Int("optionCount", optionCount))
}
