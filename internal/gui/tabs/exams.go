...
func Exams(gtx layout.Context, th *themeUI.Theme, selectedExamID *uint, db *gorm.DB, tm *tabmanager.TabManager) layout.Dimensions {
	...
	columns := []string{"Názov", "Rok", "Počet otázok", "Možností", "Počet študentov", "Dátum", "Ukázať odpovede", "Štatistika", "Vymazať", "Vyhodnotiť", "Tlačiť", "CSV"}
	columnWidths := []float32{0.16, 0.06, 0.07, 0.06, 0.08, 0.07, 0.08, 0.08, 0.08, 0.08, 0.08, 0.05} // Adjusted widths
						...
						return material.List(th.Theme, &examList).Layout(gtx, len(exams)+1, func(gtx layout.Context, i int) layout.Dimensions {
							...
							if i == 0 {
								return layout.Flex{Axis: layout.Horizontal}.Layout(gtx,
									...
									layout.Flexed(columnWidths[11], func(gtx layout.Context) layout.Dimensions {
										return widgets.LabelBorder(gtx, th, headerSize, columns[11])
									}),
								)
							}
							exam := exams[i-1]
							...
							if printExamBtns[i-1].Clicked(gtx) {
								...
								go func() {
									...
									if err != nil {
										...
										// ensure modal is closed/unblocked even on error
										isGenerating = false
										modal.SetCloseBtnEnable = true
									} else {
										generatedPath = path
							...
							return layout.Flex{Axis: layout.Horizontal}.Layout(gtx,
								...
								layout.Flexed(columnWidths[3], func(gtx layout.Context) layout.Dimensions {
									return widgets.Body1Border(gtx, th, fmt.Sprintf("%d", exam.OptionCount))
								}),
								layout.Flexed(columnWidths[4], func(gtx layout.Context) layout.Dimensions {
