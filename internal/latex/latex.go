...
func CompileLatexToPDF(latexContent []byte) ([]byte, error) {
	...
	// Ensure temporary PDF path exists
	if err := os.MkdirAll(common.TEMPORARY_PDF_PATH, 0o755); err != nil {
		errorLogger.Error("Failed to ensure temporary PDF directory exists", slog.Group("CRITICAL", slog.String("error", err.Error())))
		return nil, err
	}
