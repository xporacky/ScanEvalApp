...
// GenerateLegendPDFBytes generates the legend PDF and returns it as bytes
func GenerateLegendPDFBytes() ([]byte, error) {
    errorLogger := logging.GetErrorLogger()
    ...
    if err != nil {
        errorLogger.Error("Failed to read legend template", "error", err.Error())
        return nil, fmt.Errorf("nepodarilo sa nacitat Legendu: %w", err)
    }

    pdfBytes, err := CompileLatexToPDF(latexContent)
    if err != nil {
        errorLogger.Error("Failed to compile legend PDF", "error", err.Error())
        return nil, fmt.Errorf("chyba kompilacie legendy: %w", err)
    }
...
// GenerateLegendPDF generates a standalone legend PDF (for the Legenda tab)
func GenerateLegendPDF() (string, error) {
    logger := logging.GetLogger()
    errorLogger := logging.GetErrorLogger()
    ...
    if err != nil {
        errorLogger.Error("Failed to read legend template", "error", err.Error())
        return "", fmt.Errorf("nepodarilo sa nacitat Legendu: %w", err)
    }

    pdfBytes, err := CompileLatexToPDF(latexContent)
    if err != nil {
        errorLogger.Error("Failed to compile legend PDF", "error", err.Error())
        return "", fmt.Errorf("chyba kompilacie legendy: %w", err)
    }

    dirPath, err := config.LoadLastPath()
    if err != nil {
        errorLogger.Error("Failed to load save path", "error", err.Error())
        return "", fmt.Errorf("chyba nacitania cesty: %w", err)
    }

    absDirPath, err := filepath.Abs(dirPath)
    if err != nil {
        errorLogger.Error("Failed to convert path", "error", err.Error())
    ...
    if err := os.WriteFile(outputPath, pdfBytes, common.FILE_PERMISSION); err != nil {
        errorLogger.Error("Failed to save legend PDF", "error", err.Error())
        return "", fmt.Errorf("chyba ukladania PDF: %w", err)
    }

    logger.Info("Legend PDF generated successfully", "output_path", outputPath)
    return outputPath, nil
}

// ParallelGeneratePDFsWithLegend generates PDFs with a legend page before each student's sheets
func ParallelGeneratePDFsWithLegend(db *gorm.DB, examID uint) (string, error) {
    logger := logging.GetLogger()
    errorLogger := logging.GetErrorLogger()

    logger.Debug("Starting PDF generation with legend for each student", "exam_id", examID)

    // First generate the legend PDF bytes (will be reused for each student)
    legendBytes, err := GenerateLegendPDFBytes()
    if err != nil {
        errorLogger.Error("Failed to generate legend", "error", err.Error())
        return "", err
    }

    // Save legend to a temporary file (will be reused)
    legendPath := filepath.Join(common.TEMPORARY_PDF_PATH, "legend_temp.pdf")
    if err := os.WriteFile(legendPath, legendBytes, common.FILE_PERMISSION); err != nil {
        errorLogger.Error("Failed to save legend PDF", "error", err.Error())
        return "", err
    }
    defer os.Remove(legendPath) // Clean up after
    ...
    logger.Debug("Starting parallel PDF generation with legend for each student",
        "option_count", exam.OptionCount,
        "template_path", templatePath,
        "num_rooms", len(rooms))
    ...
    for _, room := range rooms {
        ...
        for _, student := range students {
            ...
            go func(student models.Student) {
                ...
                // Generate student's test sheet PDF
                latexTemplate, err := os.ReadFile(templatePath)
                if err != nil {
                    generationErrMutex.Lock()
                    if generationErr == nil {
                ...
                // Save student PDF to temporary file
                studentPDFPath := filepath.Join(common.TEMPORARY_PDF_PATH, fmt.Sprintf("student_%d.pdf", student.ID))
                if err := os.WriteFile(studentPDFPath, studentPDF, common.FILE_PERMISSION); err != nil {
                    generationErrMutex.Lock()
                    if generationErr == nil {
                ...
                // Create combined PDF: Legend + Student Sheets
                combinedPath := filepath.Join(common.TEMPORARY_PDF_PATH, fmt.Sprintf("combined_%d.pdf", student.ID))
               
                // Merge legend first, then student PDF
                if err := MergePDFs(legendPath, studentPDFPath, combinedPath); err != nil {
                    generationErrMutex.Lock()
                    if generationErr == nil {
                        generationErr = err
                    }
                    generationErrMutex.Unlock()
                    errorLogger.Error("Error merging legend with student PDF", "student_id", student.ID, "error", err.Error())
                    return
                }
                ...
                if !mainPDFSet {
                    mainPDFPath = combinedPath
                    mainPDFSet = true
                    logger.Info("Set initial main PDF for student", "student_id", student.ID)
                } else {
                    // Merge the new combined PDF with the existing main PDF
                    mergedPDFPath := filepath.Join(common.TEMPORARY_PDF_PATH, "merged_temp.pdf")
                    if err := MergePDFs(mainPDFPath, combinedPath, mergedPDFPath); err != nil {
                        generationErrMutex.Lock()
                        if generationErr == nil {
                            generationErr = err
                        }
                        generationErrMutex.Unlock()
                        errorLogger.Error("Error merging PDFs", "student_id", student.ID, "error", err.Error())
                        return
                    }
                    ...
                    if err := os.Rename(mergedPDFPath, mainPDFPath); err != nil {
                        ...
                        generationErrMutex.Unlock()
                        errorLogger.Error("Error updating main PDF", "student_id", student.ID, "error", err.Error())
                        return
                    }

                    // Clean up temporary files
                    go func() {
                        os.Remove(studentPDFPath)
                ...
                processedCount++
                studentDuration := time.Since(studentStartTime)
                logger.Debug("Generovanie PDF s legendou pre študenta",
                    "spracovaných", processedCount,
                    "celkovo", len(students),
                    "exam", exam.Title,
                    "id študenta", student.ID,
    ...
    safeTitle := common.SanitizeFilename(exam.Title)
    finalPDFPath := filepath.Join(absDirPath, fmt.Sprintf("%s%d_with_legend.pdf", safeTitle, exam.ID))

    // Copy the final merged PDF to the destination
    srcFile, err := os.Open(mainPDFPath)
    if err != nil {
        errorLogger.Error("error opening source PDF", "error", err.Error())
        return "", err
    ...
    duration := time.Since(startTime)
    logger.Debug("Celkový čas generovania PDF s legendou pre každého študenta", "duration", duration)
    ...
    logger.Info("Výsledné PDF s legendou pre každého študenta úspešne uložené do", "output_PDF_path", absFinalPDFPath)
