...
func (a *App) ExportExamAnswersCSV(examID uint) (string, error) {
    if a.dbErr != nil {
        return "", a.dbErr
    }
    if a.db == nil {
    ...
    template := csv.ExamTemplate{
        Title:             exam.Title,
        SchoolYear:        exam.SchoolYear,
        DateTime:          exam.Date.Format("02.01.2006"),
        QuestionCount:     exam.QuestionCount,
        OptionCount:       exam.OptionCount,
    ...
    if exam.IsMultiDay {
        questions := strings.TrimSpace(exam.Questions)
        subgroups := map[string]string{}
        if strings.HasPrefix(questions, "{") {
            if err := json.Unmarshal([]byte(questions), &subgroups); err != nil {
                subgroups = map[string]string{"A": questions}
            }
        } else if questions != "" {
            subgroups["A"] = questions
        }
        template.SubgroupAnswers = subgroups
    } else {
        answers := strings.Split(exam.Questions, "")
        for len(answers) < exam.QuestionCount {
